...
{
    public partial class frmImportStockMaster: Form
    {
        ...
        string[] columnsValuesSearch = new string[] { "WarehouseId", "SupplierId", "Note"};
        string[] columnsTextSearch = new string[] { "Mã kho hàng", "Mã ncc", "Ghichú" };
        ...
        private void menuIn_Click(object sender, EventArgs e)
        {
            ...
            frmRptImportStock frm = new frmRptImportStock(dtgvMaster.Rows[dtgvMaster.CurrentCell.RowIndex].Cells[0].Value.ToString());
            frm.ShowDialog();
        }
        ...
        private void menuCopy_Click(object sender, EventArgs e)
        {
            ...
            {
                isCopy = true;
                frmEditImportStockMaster frm = new frmEditImportStockMaster();
                frm.Text = "Copy";
        ...
        private void menuXoa_Click(object sender, EventArgs e)
        {
            ...
            if (isDelete)
            {
                ...
                ListEdit.Delete("ImportStockDetail", "ImportStockMasterId", dtgvMaster.Rows[dtgvMaster.CurrentCell.RowIndex].Cells[0].Value.ToString());
                ListEdit.Delete("ImportStockMaster", "Id", dtgvMaster.Rows[dtgvMaster.CurrentCell.RowIndex].Cells[0].Value.ToString());
                LoadData();
            }
        }
        ...
        {
            string ImportStockMasterId = "";
            Lib.CssDatagridview.LoadData(binSource, dtgvMaster, strQueryMaster + KeySearch ?? "", columnsNameMaster, widthMaster);
            if (dtgvMaster.Rows.Count > 0)
            {
                ImportStockMasterId = " WHERE ImportStockMasterId = '"+ dtgvMaster.Rows[dtgvMaster.CurrentCell.RowIndex].Cells[0].Value.ToString() + "' ";
            }
            else
            {
                ImportStockMasterId = " WHERE 1 = 0 ";
            }    
            Lib.CssDatagridview.LoadData(null, dtgvDetail, strQueryDetail + ImportStockMasterId, columnsNameDetail, widthDetail);
        }
        ...
        private void dtgvMaster_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.RowIndex >= 0)
            {
                ...
                string ImportStockMasterId = " WHERE ImportStockMasterId = '" + row.Cells[0].Value.ToString() + "'";
                Lib.CssDatagridview.LoadData(null, dtgvDetail, strQueryDetail + ImportStockMasterId, columnsNameDetail, widthDetail);
            }
        }
    }
