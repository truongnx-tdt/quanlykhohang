...
namespace PMQuanLyHangTonKho.Lib
{
    public class ListEdit
    {
        ...
        public static void LoadFormSearchListV2(string[] columnsValue, string[] columnsText, Point point, out string KeySearch, Boolean isVoucher = false)
        {
            KeySearch = "";
            string KeySearch2 = "";
            Form frm = new Form();
            ...
            btnSearch.Location = new Point(frm.ClientSize.Width - btnSearch.Width - 14, frm.ClientSize.Height - btnSearch.Height - 8);
            btnSearch.Text = "Lọc dữ liệu";
            btnSearch.Click += (sender, e) => BtnSearch_ClickV2(frm, out KeySearch2);
            frm.Controls.Add(btnSearch);
            frm.ShowDialog();
            KeySearch = KeySearch2;
        }
        private static void BtnSearch_ClickV2(Form frm, out string KeySearch)
        {
            KeySearch = " WHERE 1=1 ";
            foreach (Control control in frm.Controls)
            {
                ...
                else if (control is DateTimePicker)
                {
                    ...
                    {
                        KeySearch += " AND CONVERT(date, DateCreate) BETWEEN '" + ((DateTimePicker)control).Value.ToString("yyyyMMdd") + "' ";
                    }
